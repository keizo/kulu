...
def read():
    ...
    try:
        ...
        # no session in db. it probably expired or something -- they are anonymous, but accept cookies.
        new_sid = _generate_id()
        user = mod.user.anonymous_user(sid=new_sid)
        return user
...
def write(key, value='',user=''):
    ...
    if not user: user = mod.user.anonymous_user(sid=key)
    if not web.cookies():
